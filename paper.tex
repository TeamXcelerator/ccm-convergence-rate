...
The entire open content of the archimedean rate is thereby confined to one
statement. We record it formally, in three clauses that separate the objects
involved: the eigenvector tail of the full form, the archimedean
identification of its rate, and the transfer to the zero.
...
\item[(i)] \emph{(Gevrey tail, full form.)} There exist $c(\lambda^2)>0$ and
  ...
  \]
\item[(ii)] \emph{(Archimedean identification.)} For the archimedean-only
  localized form the tail is purely exponential ($q=1$) with rate
  $\delta\cdot(2\pi/L)$, $\delta=\pi/4$; equivalently,
  $c(\lambda^2)\,L/(2\pi)\to\delta$ and $q\to1$ as the prime content is
  removed.
...
$\delta\approx0.785$) is then the first data point for the prime modulation
that clause (ii) asserts, not a discrepancy. Clause (iii) carries the doubling:
summing the tail of clause (i) over the discarded modes bounds the transform
difference near $\gamma_1$ only at the single exponent $c(\lambda^2)N^{q}$,
and Lemma~\ref{lem:rouche} then moves the zero at that same rate, while the
observed displacement decays at twice it. The mechanism, whether first-order
...
with no caveat. The content lies in the expected size of the right-hand side:
evaluating $R[k_\lambda]$ through the semilocal trace formula, with the
uniform prolate estimates of Meixner--Sch\"afke as used in
\cite[Lem.~7.2]{CCM2025} and the Fuchs asymptotic~\cite{Fuchs1964}, is
expected to yield $R[k_\lambda]\le C\,\lambda^{9}e^{-4\pi\lambda^2}(1+o(1))$,
hence
\[
  D_{\mathrm{eig}}(\lambda^2)\;\ge\;K\lambda^2-\frac{9}{2\ln10}\ln\lambda^2
  -\log_{10}C
\]
whenever $\varepsilon^\infty(\lambda)>0$ so that the digit reading
$D_{\mathrm{eig}}=-\log_{10}\varepsilon^\infty$ is defined. That evaluation
is the \emph{named} open lemma behind the floor's leading rate; each
ingredient (variational principle, Meixner--Sch\"afke, Fuchs) is classical,
and this is the precise sense in which the rate is T2. The reverse inequality,
...
$D_{\mathrm{Primes}}$ leading $K$: floor lower bound & \textbf{T2} & evaluation of $R[k_\lambda]$ (Prop.~\ref{prop:variational}) & named lemma \\
$D_{\mathrm{Primes}}$ leading $K$: ceiling unbeatable & open & archimedean--prime cancellation & new analysis \\
$D_{\mathrm{Primes}}$ floor slope $9/2$ & \textbf{T2} & same $R[k_\lambda]$ evaluation; primes provably inert & named lemma \\
...
digit- and eigenvalue-space rates), and the zero-displacement transfer of
Lemma~\ref{lem:rouche}) stands on its own; the variational bound of
...
\emph{named lemmas} (the trace-formula evaluation of
$R[k_\lambda]$ and the first-order Szeg\H{o} asymptotics for the log symbol)
