...
of one prolate-type spectral edge. Observed at finite working precision $P$, the
...
\begin{remark}[The two channels are one edge]\label{rem:oneedge}
The mode and prime channels are the two ends of a single spectral edge. By
\eqref{eq:nmodes}, $D_{\mathrm{nModes}}=D_{\mathrm{Primes}}\cdot g$ with
$g=\tanh\!\bigl((c\,N/(\lambda^2\ln\lambda^2))^{q}\bigr)\uparrow1$ as
$N\to\infty$, so $D_{\mathrm{Primes}}=\lim_{N\to\infty}D_{\mathrm{nModes}}$ is not
a separate budget but the $N\to\infty$ ceiling of the mode ramp. Both are
features of one prolate-type band edge: the ramp is its finite-$N$ transition
(the Landau--Widom plunge, Section~\ref{sec:nmodes}), the ceiling its extreme
tail (the prolate asymptotic, Section~\ref{sec:primes}). Equivalently the digit
law is a single edge function $D_{\mathrm{edge}}(N,\lambda^2)=
D_{\mathrm{Primes}}\cdot g$ capped by precision,
$D=\min(D_{\mathrm{Wprec}},D_{\mathrm{edge}})+O(1)$.
\end{remark}
